  ...
  if (dataType == selectType(value)) {
    ...
    if (it != fieldValues.end()) {
      throw std::runtime_error(
          "Attempted to insert a row that conflicts with an existing entry. "
          "Duplicate entries are not allowed.");
    }
    fieldValues.insert({id, value});
    return;
  }
  throw std::runtime_error(std::format("The provided value {} is incompatible "
                                       "with the data type of the column {}.",
                                       value, name));
}

auto Column::insertDefaultValues(const int ids) -> void {
  auto const &defaultValue = defaultValues[dataType];
  for (int id = 1; id < ids; id++) {
    fieldValues.insert({id, defaultValue});
  }
}

auto Column::setName(std::string const &newName) -> void { name = newName; }

auto Column::getName() const -> std::string { return name; };

auto Column::eraseFieldValues() -> void { fieldValues.clear(); };

auto Column::findValue(std::string const &value) const {
  return std::ranges::find_if(fieldValues, [value](auto const &pair) -> bool {
    return value == pair.second;
  });
};

auto Column::getFilteredRows(std::string const &value,
                             std::string const &operand) const
    -> std::vector<int> {
  static const std::unordered_map<
      std::string,
      std::function<std::vector<int>(const Column &, const std::string &)>>
      operators = {{"=", [](const Column &column,
                            const std::string &val) { return column == val; }},
                   {"!=", [](const Column &column,
                             const std::string &val) { return column != val; }},
                   {">", [](const Column &column,
                            const std::string &val) { return column > val; }},
                   ...
                   {">=", [](const Column &column,
                             const std::string &val) { return column >= val; }},
                   {"<=", [](const Column &column, const std::string &val) {
                      return column <= val;
                    }}};
...
auto Column::updateValue(int const &id, std::string const &newValue) -> void {
  if (selectType(newValue) != dataType) {
    throw std::runtime_error(
        std::format("The provided value {} is incompatible with the data type "
                    "of the column {}.",
                    newValue, name));
  }
...
auto Column::printHeader(const size_t width) -> std::string {
  ...
  header << "| " << getName();
  for (size_t i = getName().size(); i < width; i++) {
    header << " ";
  }
  header << " |" << '\n';
  return header.str();
...
auto Column::printRow(std::string const &value, const size_t width)
    -> std::string {
...
auto Column::calculateWidth() const -> size_t {
  size_t width = name.size();
  for (auto const &[id, value] : fieldValues) {
    width = std::max(width, value.size());
  }
  return width;
}

auto Column::setFieldValues(const std::map<int, std::string> &newFieldValues)
    -> void {
  fieldValues = newFieldValues;
}

auto Column::getOrderedRows() const -> std::vector<int> {
  auto comp = [this](std::pair<int, std::string> const &p1,
                     std::pair<int, std::string> const &p2) -> bool {
    if (dataType == "INT") {
  ...
  for (auto const &pair : fieldValues) {
